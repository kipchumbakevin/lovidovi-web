...
class CrushyCrushCoontroller extends Controller
{
    public function insert(Request $request)
    {
        ...
        $nottt = Crush::where('sender_phone',$ownerp)->get();
        $ss = [];
    ...
    public function fetch(Request $request)
    {
        ...
        $notify = Crush::where('receiver_phone',$ownerp)->latest()->get();
        return $notify;
    }
    ...
    {
        $ownerp = $request['ownerphone'];
        $notif = Crush::where('receiver_phone',$ownerp)->where('status',false)->count();
        return response()->json([
            'num' => $notif,
        ],201);
    }
    ...
    {
        $notif = Crush::where('id',$request->id)->first();
        $notif->delete();
        return response()->json([
    ...
    {
        $ownerp = $request['ownerphone'];
        $nnn = Crush::where('receiver_phone',$ownerp)->where('status',false)->get();
        foreach ($nnn as $dff){
            $dff->update([
                'status'=>true
            ]);
